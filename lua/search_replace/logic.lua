...
function M.apply_blocks(content)
	...
	while pos <= #content do
		...
		local blocks = {}
		if search_s then
			table.insert(blocks, { type = "search", s = search_s, e = search_e })
		end
		if create_s then
			table.insert(blocks, { type = "create", s = create_s, e = create_e })
		end
		if delete_s then
			table.insert(blocks, { type = "delete", s = delete_s, e = delete_e })
		end
		if move_s then
			table.insert(blocks, { type = "move", s = move_s, e = move_e })
		end
		...
		elseif is_delete then
			...
			if file_or_buf_exists(full_path) then
				...
				if choice == 1 then
					...
					vim.notify(
						"[search_replace.nvim] ⏭️ Skipped DELETE (User rejected): " .. current_path,
						vim.log.levels.INFO
					)
				end
			else
				vim.notify(
					"[search_replace.nvim] ❌ File not found for DELETE: " .. current_path,
					vim.log.levels.ERROR
				)
			end
		...
		elseif is_move then
			...
			if not is_safe_path(new_full_path) then
				vim.notify(
					"[search_replace.nvim] Skipping MOVE: Destination outside project root (" .. new_path_raw .. ").",
					vim.log.levels.ERROR
				)
				pos = close_e + 1
				goto continue_loop
			...
			if file_or_buf_exists(full_path) then
				...
				if file_or_buf_exists(new_full_path) then
					...
					if choice ~= 1 then
						proceed = false
						vim.notify(
							"[search_replace.nvim] ⏭️ Skipped MOVE (User rejected overwrite): " .. new_path_raw,
							vim.log.levels.INFO
						)
					end
				end
